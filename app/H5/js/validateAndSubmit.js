...
function myObj(dispatch, getState,familyNameLst,loadFlg){
    const sta = getState()[familyNameLst[0]].present,
        list = familyNameLst.slice(1,familyNameLst.length),
        nowProps=undidObj(sta,list,{});
   return{
       props:{source:nowProps.source},
       setState :(result={})=>{
           const sta = getState()[familyNameLst[0]].present,
                 list = familyNameLst.slice(1,familyNameLst.length);
           undidObj(sta,list,result,loadFlg);
           dispatch({type:'SUBMITE',state:sta});
       }
   }
};
/**
 ...
 * @param result
 */
function undidObj(getState,familyNameLst,result,loadFlg){
